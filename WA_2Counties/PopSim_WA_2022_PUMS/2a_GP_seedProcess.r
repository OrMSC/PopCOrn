...
# Alex Bettinardi 3-18-24 edited for WA PUMAs

# set group quarters data directory
gpData <- "data"

# read in household table
hh <- read.csv(paste0(gpData,"/psam_h53.csv"),as.is=T)
rownames(hh) <- hh$SERIALNO

# add 2010 PUMA filed (for 2020 PUMA records)
PUMA10s <- c(11000,11101:11104)
names(PUMA10s) <- c(24100,21103,21104,21102,21101)

hh <- hh[hh$PUMA10 %in% PUMA10s | hh$PUMA20 %in% as.numeric(names(PUMA10s)),]

hh$PUMA10[hh$PUMA10==-9] <- PUMA10s[as.character(hh$PUMA20[hh$PUMA10==-9])]
hh$PUMA <- hh$PUMA10

# read in person table
per <- read.csv(paste0(gpData,"/psam_p53.csv"),as.is=T)
# subset to just filtered hhs (in the 5 PUMAs)
per <- per[per$SERIALNO %in% rownames(hh),]
# move PUMA10 field over
per$PUMA <- hh[per$SERIALNO,"PUMA"]
