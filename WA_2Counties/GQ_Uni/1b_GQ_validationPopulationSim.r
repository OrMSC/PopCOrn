...
#
# JXR updated the script for GQ output validation on 9/25/2019
#
##############################################################################################################
...
geographyList   <- c("BLOCK")      #TAZ Flag removed, as GQ does not use
plotGeographies <- c("BLOCK")              #TAZ Flag removed, not needed in GQ

geogXWalk       <- read.csv(paste(popSimDir, "/data/geo_cross_walk.csv", sep = "/"))
columnMap       <- read.csv(paste(validationDir, "/data/GQ_columnMapPopSim.csv", sep = "/"))

seed_households <- read.csv(paste(popSimDir, "/data/GQ_seed_households.csv", sep = "/"))
seed_col        <- c("PUMA", "hhnum", "GQWGTP")
...
#summary3           <- read.csv(paste(popSimDir, "/output/summary_TAZ.csv", sep = "/"))      Removed, Not needed for GQ
summary1           <- read.csv(paste(popSimDir, "/output/final_summary_BLOCK.csv", sep = "/"))
...
write.csv(stats, paste0("output/", scenario, "GQ_PopulationSim_stats.csv"), row.names = FALSE)
...
ggsave(file=paste0("plots/", scenario, "GQ_PopulationSim_Convergance_sdev.jpeg"), width=8,height=10)
...
ggsave(file=paste0("plots/", scenario, "GQ_PopulationSim_Convergance_PRMSE.jpeg"), width=8,height=10)
...
uniformity <- seed_households[seed_households$GQWGTP>0, seed_col] %>%
  left_join(summary_hhid, by = c("hhnum" = "hh_id")) %>%
  mutate(FINALWEIGHT = ifelse(is.na(FINALWEIGHT), 0, FINALWEIGHT)) %>%
  mutate(EXPANSIONFACTOR = FINALWEIGHT/GQWGTP) %>%
...
write.csv(uAnalysisPUMA, "output/GQ_uniformity.csv", row.names=FALSE)
}
